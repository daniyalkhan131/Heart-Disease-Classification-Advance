...
def main():
    ...
    model_file = home_dir.as_posix()+'/models/pipe_model.joblib'
    # Load the model.
    ...
    X_train = np.load(data_path+'/interim/X_train.npy')
    y_train = np.load(data_path+'/interim/y_train.npy')
    # feature_names = X_train.columns.to_list()
    feature_names=list(range(1,8))

    # test_features = pd.read_csv(data_path + '/test.csv')
    X_test = np.load(data_path+'/processed/X_test.npy')
    y_test = np.load(data_path+'/processed/y_test.npy')
